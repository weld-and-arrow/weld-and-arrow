/-
================================================================================
  WeldAndArrow.Exposition.Glossary
  Canonical glossary data, structural checks, and rendering
================================================================================
...
def glossary : List GlossaryEntry := [
  ...
  { term := "grid"
    kind := .coinage .theory
    gloss := "A grid is the compatibility name for a deliberately small package of independent occurrence, response, placement, and conditioning readings over one designatum carrier. It supplies the apparatus for welds, grades, delivery, and diagnostics without pretending to be the whole doctrine."
    anchors := [``CoreReadings]
    seeAlso := ["floor"] },
  { term := "axiomatic reconstruction"
    ...
    anchors := [``CoreReadings, ``Grid.Distinction.ObeysSeparateFuse]
    seeAlso := ["grid"] },
  { term := "act-grammar"
    kind := .coinage .theory
  ...
  { term := "shu"
    ...
    anchors := [``OccurrenceReading.Weld]
    seeAlso := ["act-grammar"] },
  { term := "shushō-ittō"
    kind := .canonical
  ...
  { term := "call"
    kind := .coinage .theory
    gloss := "A call is something that arrives to be answered. In the grid it is a role reading of an occurrence designatum, the thing a response addresses or fails to address."
    anchors := [``OccurrenceReading.Weld.call,
      ``Grid.RecordedUtterance.answersCall]
    seeAlso := ["grid"] },
  { term := "response"
    kind := .coinage .theory
    gloss := "A response is what a being produces when it answers a call. In the grid it is a role reading of an occurrence designatum and the visible side of function."
    anchors := [``OccurrenceReading.Weld.response, ``Grid.MountsAt]
    seeAlso := ["call"] },
  { term := "field"
    kind := .coinage .theory
  ...
  { term := "weld"
    kind := .coinage .theory
    gloss := "A weld is an occurrence designatum selected by an occurrence reading. Its agent, call, and response are role readings of that one occurrence, so the act's index is made by the act rather than stored before it."
    anchors := [``OccurrenceReading.Weld, ``Grid.Weld, ``Grid.index]
  ...
  { term := "seed"
    ...
    anchors := [``Config, ``Config.tendency, ``Grid.relabel_rePitch]
  ...
  { term := "call/response"
    ...
    anchors := [``OccurrenceReading.Weld.call,
      ``OccurrenceReading.Weld.response, ``Grid.RecordedUtterance]
  ...
  { term := "retrospective soul"
    ...
    anchors := [``Grid.DirectedConvention.BeingConvention.GridConvention.sowingReapingRow_not_freeze,
      ``Grid.no_agent_recovery_of_field_collision]
  ...
  { term := "soul-guard"
    ...
    anchors := [``Grid.no_agent_recovery_of_field_collision]
    seeAlso := ["field", "weld"] },
  ...
  { term := "Modal Realism"
    ...
    anchors := [``Disclaimer.modalRealismFreeze]
...
def glossaryBody : String :=
  "# Glossary\n\n" ++
  "Generated from `WeldAndArrow/Exposition/Glossary.lean` by `lake exe exposition_gen`. " ++
...
def glossaryDoc : Exposition.Doc :=
  { id := Exposition.glossaryRef.id
    title := Exposition.glossaryRef.title
    output := Exposition.glossaryRef.output
    source := "WeldAndArrow/Exposition/Glossary.lean"
    summary := Exposition.glossaryRef.summary
